<%@ page language="C#" autoeventwireup="true" inherits="Guest_Bar_PostContent, App_Web_yu5ymvsg" masterpagefile="~/Guest/Guest.master" validaterequest="false" enableEventValidation="false" viewStateEncryptionMode="Never" %>
...
<div class="postName text-center"><a <%#GetHref() %>><%#Eval("CUName") %></a></div>
<div class="buy_div group<%#Eval("GroupID") %>" style="display:none;"><button type="button" class="btn btn-danger btn-sm" onClick="ContactFun('<%#Eval("CUser") %>');">联系医生</button></div>
<div class="groupName"></div>
<div class="msg_rel_div">
<div class="msg_rel_border"></div> 
<div class="msg_rel_main">
<div class="col-md-4 col-xs-5 padding5"><a <%#GetHref() %>><img class="img-responsive center-block" src='<%#GetUserFace() %>' onerror="this.src='/Images/userface/noface.gif';" /></a></div>
<div class="col-md-8 col-xs-7 padding5">
<div class="text-right msgmain_div_close"><a href="javascript:;" onclick="CloseUserInfo()"><span class="glyphicon glyphicon-remove"></span></a></div>
<ul class="list-unstyled">
<li><span>积分：</span>@UserExp</li>                             
<li><span>性别：</span>@UserSex</li>
<li>@GroupName</li>
<li><span>生日：</span>@UserBirth</li>
...
<td><div class="width200">
<input type="text" id="ReceUser" class="form-control width200"/>
...
联系医生需付费，点击下方确认按钮进行付费，查看医生的联系方式！
</div>
<div class="modal-footer">
<button type="button" class="btn btn-primary" onClick="sureBuy();">确认</button>
...
<asp:Content runat="server" ContentPlaceHolderID="Form">
<form id="form_p" name="form_p" method="post" action="/Cart/FastOrder.aspx">
<input type="hidden" value="商品名称" id="Proname" name="Proname" />
<input type="hidden" value="国外卖家" id="ProSeller" name="ProSeller" />
<input type="hidden" value="100" id="Price" name="Price" />
<input type="hidden" id="Pronum" name="Pronum" value="1"  />
<input type="hidden" value="1" id="ProClass" name="ProClass" />
<input type="hidden" value="<%Call.Label("{ZL.Label id=\"获取指定帖子的用户ID\" GID=\"{$GetRequest(ID)$}\" /}");%>" id="Proinfo" name="Proinfo" />
...
#privatediv .modal-dialog{ width:700px;}
...
<script> 
    ...
    function UInfoShow(obj){
        ...
        $(".msg_rel_div").hide();
        $uinfodiv.show();
        buser.GetBarUInfo(1,function (data) {
	...
	{
		var un="<%Call.Label("{ZL:GetuserName()/}");%>";
		if(un=="匿名用户"||un=="")
			window.location.href="/User/Login.aspx?ReturnUrl=/PItem?id=<%Call.Label("{$GetRequest(ID)$}");%>";
		else
		{
			$("#Proinfo").val(id);
			$('#myModal').modal('show')
	...
	$(".user_left>ul>li>a").click(function(){
